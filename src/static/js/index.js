function cam_html(ip) {
    ...
    tmp = '<img src="./stream/' + ip + '"' + 'id=' + ip + '>'
    index.innerHTML = '<a href="#!" id="' + ip + '" class="list-group-item list-group-item-action frame">' + tmp + '</a>';
    return index;
}

function img_html(ip) {
    var index = document.createElement('div');
    console.log('Img_html', ip)
        // index.innerHTML = '<img src="./thumbnail/' + ip + '/frame.jpg" class="list-group-item list-group-item-action ' + 'id=' + ip + '>'
    tmp = '<img src="./thumbnail/' + ip + '/frame.jpg"' + 'id=' + ip + '>'
    index.innerHTML = '<a href="#!" id="' + ip + '" class="list-group-item list-group-item-action frame">' + tmp + '</a>';
    return index;
}

const cam = ['cam_01', 'cam_10', 'cam_12', 'cam_09', 'cam_03', 'cam_04', 'cam_05', 'cam_06', 'cam_07', 'cam_08', 'cam_02', 'cam_11']
...
function update_camera() {
    ...
    for (let i = 3; i < cam.length; i++) {
        document.getElementById('cam_list').appendChild(img_html(cam[i]));
    }
}
...
$("#cam_list").on("click", function(event) {
    ...
    videoElement.setAttribute('src', "./stream/" + event.target.id);
    console.log("./stream/" + event.target.id);
...
pingMap.forEach((item, index) => {
    item.addEventListener('click', () => {
        ...
        videoElement.setAttribute('src', "./stream/" + cam[index]);
        document.documentElement.scrollTop = 0;
        // loadXMLDoc()
        send(cam[index])
